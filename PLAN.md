...
```text
设计任务状态机与失败重试机制，支持 HTTP 回调超时控制、手动触发和完整执行日志记录。
```
...
```text
基于 Go 实现分布式定时任务调度系统，支持 Cron 表达式、任务 CRUD、HTTP Callback、手动触发、执行日志、失败重试和超时控制。使用 Redis ZSet 管理任务触发时间，并通过 Redis Lua 脚本保证到期任务原子获取与删除；设计幂等键和任务状态机，解决多实例部署下任务重复执行和宕机恢复问题。
```
